...
```bash
# 1. 克隆项目
git clone https://github.com/samni728/tts_md2audio.git
cd tts_md2audio
...
# 2. 使用国内版本启动（自动配置镜像加速器）
./start-china.sh

# 3. 访问服务
# 浏览器打开: http://your-server-ip:5055
```
...
├── Dockerfile.china            # 国内版本 Dockerfile
├── docker-compose.yml          # Docker Compose 配置
├── docker-compose.china.yml    # 国内版本 Docker Compose 配置
├── requirements.txt            # Python 依赖
├── daemon.json                 # Docker 镜像加速器配置
├── start.sh                    # 标准启动脚本
├── start-china.sh              # 国内服务器启动脚本
├── stop.sh                     # 停止脚本
├── DOCKER_DEPLOY.md            # Docker 部署文档
├── CHINA_DEPLOY.md             # 国内部署文档
└── README.md                   # 说明文档
```
...
- 转换后的文件会覆盖同名文件
- 建议单个文件不要过大，以确保转换稳定性
- Docker 部署时确保端口 5055 未被占用
- 国内服务器建议使用 `./start-china.sh` 启动
- 上传的文件会保存在 `uploads/` 目录中，请定期清理

## 📚 相关文档

- [Docker 部署详细指南](DOCKER_DEPLOY.md)
- [国内服务器部署指南](CHINA_DEPLOY.md)
